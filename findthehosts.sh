...
export EC2KEYNAME=`terraform output --json | jq -r '.key_name.value'`
...
echo scp -i $EC2KEYNAME $EC2KEYNAME azureuser@$JUMPHOSTIP0:~/$EC2KEYNAME
echo ssh -i $EC2KEYNAME azureuser@$JUMPHOSTIP0
...
echo connect to jumphost at with
echo scp -i $EC2KEYNAME $EC2KEYNAME azureuser@$JUMPHOSTIP1:~/$EC2KEYNAME
echo ssh -i $EC2KEYNAME azureuser@$JUMPHOSTIP1
echo when the ansible run is complete Juiceshop and Grafana should be available at
